 ...
 * SSHMisc
 *
 * This file provides miscellaneous static functions:
 *
 *	crc
 *	concatenate
 ...
 *	XORByteArrays
 *
 * This code was part of "The Java Ssh Applet" by Cedric Gourio
...
/**
 * This class provides miscellaneous static methods for use in the
 * Java SSH Client.
 */
public class SSHMisc {

    /** Get a nonzero random byte for use as padding at the start of an
     * SSH packet.
     */
    static public byte getNonZeroRandomByte(MessageDigest md5_) {
	...
	md5_.reset();
        ...
        return getNonZeroRandomByte(md5_);	// recursive call
    }

    /** Returns a new byte array which is the concatenation of the
     * two byte arrays supplied.
     */
     ...
     * Each element of the new array is formed by XORing the corresponding 
     * elements of the supplied arrays.
     */
    static public byte[] XORByteArrays(byte[] a, byte[]  b) {
        if (a==null || b == null)
	    return null;
    ...
    /**
     * This table simply represents the results of eight shift/xor operations for
     ...
     */
    public static String debug(byte[] bytes_) {
	StringBuffer buf = new StringBuffer();
	for (int i=0; i<bytes_.length; i += 8) {
	    buf.append(row2string(bytes_, i));
	}
	return buf.toString();
    }

    private static String row2string(byte[] bytes_, int offset_) {
	StringBuffer buf = new StringBuffer();
	for (int i=0; i<8; i++) {
	    if ((i + offset_) < bytes_.length) {
		int hinibble = (bytes_[i + offset_] >> 4) & 0xf;
		int lonibble = bytes_[i + offset_] & 0xf;
		buf.append(' ');
		buf.append(hexDigit(hinibble));
		buf.append(hexDigit(lonibble));
	    }
	    else
		buf.append("   ");
	}
